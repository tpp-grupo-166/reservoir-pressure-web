...
def register(user_data: UserRegister, system: System = Depends(get_system)) -> UserResponse:
    try:
        user = system.register_user(user_data.email, user_data.password)
...
def login(user_data: UserLogin, system: System = Depends(get_system)) -> Token:
    try:
        token = system.login(user_data.email, user_data.password)
...
def get_current_user_info(
    ...
    try:
        user = system.get_user_from_token(token)
        return UserResponse(id=user.id, email=user.email)
...
def get_all_users(system: System = Depends(get_system)) -> list[UserResponse]:
