...
class RidgeModel(Model):
    ...
    def train(self) -> None:
        ...
        Xtr, ytr, gtr = _prepare(norne, pvt, TRAIN_SIMS)
        Xte, yte, _ = _prepare(norne, pvt, TEST_SIMS)
        ...
        joblib.dump({
            ...
            "mean_delta_curve": _mean_delta_curve(norne, TRAIN_SIMS).astype(np.float32),
            "metrics": metrics,
...
def _prepare(raw: pd.DataFrame, pvt: pd.DataFrame, sim_ids: list[int]):
    ...
    for sid in sim_ids:
        ...
        xs.append(feats[FEATURE_COLUMNS].to_numpy())
...
def _mean_delta_curve(raw: pd.DataFrame, sim_ids: list[int]) -> np.ndarray:
    """Curva de caída promedio del train (truncada al largo de la sim más corta)."""
